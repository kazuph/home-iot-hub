...
namespace hub::ble
{
    ...
    {
        std::from_chars(std::next(address.data(), 0), std::next(address.data(), 2), this->address[5], 16);
        std::from_chars(std::next(address.data(), 3), std::next(address.data(), 5), this->address[5], 16);
        std::from_chars(std::next(address.data(), 6), std::next(address.data(), 8), this->address[5], 16);
        std::from_chars(std::next(address.data(), 9), std::next(address.data(), 11), this->address[5], 16);
        std::from_chars(std::next(address.data(), 12), std::next(address.data(), 14), this->address[5], 16);
